...
bool StateMachine::open (string device, int baudrate) {
  first_valid = false;
  device_ = device;
  baudrate_ = baudrate;
  cout << "StateMachine: Opeing port " << device_ << " with baudrate: " <<
    baudrate_ << endl;

  if(serialDevice_!=NULL) {
    cout << "StateMachine: serialDevice is not NULL - deleting the device  " <<
      endl;
...
int StateMachine::write_configuration(int frequency_hz_sampling, int
    frequency_hz_filter, int flag_raw) {
...
void StateMachine::read(std::deque<sp::DisplayPackage>& packs, const
    sp::DisplayPackage& offset) {
  uint16_t actual;
  uint8_t checksum;
  uint16_t checksum2;
  sp::DisplayPackage pack;
